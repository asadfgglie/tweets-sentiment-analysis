...
def load_dataset(path: str='./training.1600000.processed.noemoticon.csv', return_dict: bool=False, sample_size: int=1000) -> Union[dict, pd.DataFrame]:
    ...
    with open(path, 'r', encoding='utf8', errors='ignore') as f:
        ...
        sampled_data = random.sample(list(reader), sample_size)  # Randomly extract sample_size pieces of data
        for target, ids, date, flag, user, text in sampled_data:
            text = re.sub(r'@\w+\s?', '', text)  # Use regular expressions to remove the @ symbol and the user name that follows it from the text
